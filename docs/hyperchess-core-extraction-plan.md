...
**Status:** Phases 0–6 complete. All six Rust crates exist, build, and are tested: `hyperchess-rules`, `hyperchess-eval`, `hyperchess-search`, `hyperchess-search-cuda`, `hyperchess-driver`, `hyperchess-wasm`. `hyperchess-wasm` merges `wasm.rs` + the standalone `hyperchess_3d` crate into one wasm-pack target — verified with a real `wasm-pack build` + Node.js smoke test against the compiled `.wasm` binary, not just `cargo build`. Known open item from that phase: bundling `wgpu` means every consumer now pays the 3D renderer's size cost (~4.5MB), even 2D-only ones — flagged for Phase 8 to potentially address via a Cargo feature split. Phase 7 (relocate `packages/*`) is next.
...
| 7 | Relocate `packages/*` | Copy `hyperchess_sdk/packages/*`, repoint `@hyperchess/wasm`, `pnpm -r test` green | Low |
