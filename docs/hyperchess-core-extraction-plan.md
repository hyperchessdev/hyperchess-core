...
**Status:** Phases 0–4 complete. `hyperchess-rules`, `hyperchess-eval`, `hyperchess-search`, `hyperchess-search-cuda`, and `hyperchess-driver` all exist, build, and are tested. §13's "one binary, subcommands" open item is resolved: `hyperchess-driver` ships one `hyperchess` binary (`play`/`perft`/`show`/`gpu-info`/`bench-eval`/`uci`), not the source repo's two separate binaries. Phase 5 (`hyperchess-driver::api`) is next — the one component with no existing analog, budget the most iteration time here per §12's suggested order of attack.
