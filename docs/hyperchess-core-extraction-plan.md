...
**Status:** Phases 0–5 complete. All five backend crates exist, build, and are tested: `hyperchess-rules`, `hyperchess-eval`, `hyperchess-search`, `hyperchess-search-cuda`, `hyperchess-driver` (now with `cli`/`uci`/`api` subcommands — `play`/`perft`/`show`/`gpu-info`/`bench-eval`/`uci`/`api`). The API driver (§6) is live and verified against both a real running server and 9 automated integration tests. Phase 6 (`hyperchess-wasm`) is next, then 7 (relocate `packages/*`) — the two phases that make visible progress on the SDK/UI side per §12's suggested order of attack.
